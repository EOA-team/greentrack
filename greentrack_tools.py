...
def annual_px_stats(dates,data,plot=False):
    ...
    # area under the indicator curve from SOG to EOS
    ...
    # OUTPUT:
    # SOG (Start of season), EOS (End of Season), LOS (Lenght of Season), 
    # AUC (Area under the curve), CM (mean of the curve), 
    # Q90 (90-th quantile), STD (Stard deviation), GS (growth slope)
    return [sogm, eosm, losm, aucm, cm, q90, stdm, gs]
...
def rasterize_from_xarray(shp_path,
                            ds,
                            field:str = '1',
                            no_data = 0,
                            dtype = None):
    """
        ...
        ds : xarray dataset 
            raster giving the grid to rasterize the shapefile
        ...
        field : str
            ...
            Default is 'None', meaning constant 1 for all polygons pixels.
    ...
    # read shpafile in the xarray crs
    vector = gpd.read_file(shp_path).to_crs(ds.crs)

    if field != '1':

        # iterator of geometry - field couples
        geom = ((geom,value) for geom, value in zip(vector.geometry, vector[field]))
    
    else:
        # geometry iteration (field is given by default value 1)
        geom = [g for g in vector.geometry]
    ...
    sx = ds.x.size
    sy = ds.y.size
    transform = from_origin(x[0], y[0], np.abs(x[1] - x[0]), -np.abs(y[1] - y[0]))
    ...
    rasterized = features.rasterize(geom,
                                    ...
                                    fill = no_data,
                                    transform = transform,
                                    all_touched = False, # False = only contained pixel centers are considered covered by the polygon
                                    default_value = 1,
                                    dtype = dtype)
...
def rasterize_shp(tx:float,
    ...
    gs = gdf.buffer(0) # to make all shape valid
    gdf.set_geometry(gs)
    ...
    s = gpd.GeoSeries.from_xy(xx.ravel(), yy.ravel(), crs="EPSG:" + str(target_crs))
...
def _proc_chunk(coords, time_vec, shm_name, shape):
    """Process a chunk of the array to compute cumulative sums."""
    
    # recall shared data cube
    existing_shm = shared_memory.SharedMemory(name=shm_name)
    data_cube_shared = np.ndarray(shape, dtype='float', buffer=existing_shm.buf)
    
    # extract current pixel chunk to process linearly
    (i_start, i_end), (j_start, j_end) = coords
    evi_chunk = data_cube_shared[i_start:i_end, j_start:j_end, :]

    # allocate arrays for output indicators
    ind_names = ['SOG','EOS','LOS','AUC','CM','Q90','STD','GS'] # IMPORTANT: keep same order as in annual_px_stat()
    indicator_chunks = {} # dictionary of indicators chunks

    for k in range(len(ind_names)):
        indicator_chunks[ind_names[k]] = np.full((i_end - i_start, j_end - j_start), np.nan, dtype='float32')
    ...
    for i in range(evi_chunk.shape[0]):
        for j in range(evi_chunk.shape[1]):
            ...
            if not np.all(np.isnan(pixel_data)):
                ...
                # list of indicators for one pixel following the order of ind_names
                indicators = annual_px_stats(time_vec,pixel_data,plot=False) 

                # assign pixel values
                for k in range(len(ind_names)):          
                    indicator_chunks[ind_names[k]][i,j] = indicators[k]  # pixel indicator value
                
    existing_shm.close()
    return (i_start, i_end), (j_start, j_end), indicator_chunks

def _update_progress(result, progress_bar, indicator_rast, chunk_npix):
    """Callback function to update shared state with the result of _proc_chunk."""

    # extract rtesults processed chunk
    (i_start, i_end), (j_start, j_end), indicator_chunks = result 

    # update all indicator maps with processed chunks
    ind_names = list(indicator_rast.keys())
    for k in range(len(ind_names)):
        indicator_rast[ind_names[k]][i_start:i_end, j_start:j_end] = indicator_chunks[ind_names[k]].copy()
...
def indicator_map(data_cube, time_vec, njobs=1, chunk_size=10,silent=False):
    ...
    -------
    Dict
        Dictionary containing a series of arrays of dimensionality (x,y) and same size of data_cube[:,:,None]
        Every key being a different indicator.

    """
    
    # PREALLOCATE arrays for output indicators
    ind_names = ['SOG','EOS','LOS','AUC','CM','Q90','STD','GS'] # IMPORTANT: keep same order as in annual_px_stat()
    indicator_rast = {} # dictionary of indicators chunks

    for k in range(len(ind_names)):
        indicator_rast[ind_names[k]] = np.full(data_cube.shape[:2], np.nan, dtype='float32')

    # LINEAR IMPLEMENTATION

    if njobs == 1:

        progress_bar = tqdm(total=data_cube.shape[0] * data_cube.shape[1],disable=silent)
        
        for i in range(data_cube.shape[0]):
            for j in range(data_cube.shape[1]):
                
                pixel_data = data_cube[i, j, :]
                ...
                if not np.all(np.isnan(pixel_data)):
                    ...
                    # assign pixel values
                    for k in range(len(ind_names)):          
                        indicator_rast[ind_names[k]][i,j] = indicators[k]  # pixel indicator value
                        
                    progress_bar.update(1)

        return indicator_rast
        ...
        nrows, ncols = indicator_rast[ind_names[0]].shape
        ...
        # Create shared memory for input data_cube
        ...
        with tqdm(total=total_chunks * chunk_npix,disable=silent) as progress_bar:
            _update_progress_with_bar = partial(_update_progress, progress_bar=progress_bar, indicator_rast=indicator_rast, chunk_npix=chunk_npix)
        ...
        return indicator_rast # dictionary of raster maps
